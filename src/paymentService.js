 ...
 * Wraps @babuperumana/upipg-new with SQLite persistence and VPA enrichment.
 */

const { EventEmitter } = require('events');
const { UpiPG } = require('@babuperumana/upipg-new');
...
const bharatpeAxios = axios.create({ httpAgent, httpsAgent: agent, timeout: 15000 });
...
class PaymentService extends EventEmitter {
  ...
  registerMerchant(merchant) {
    ...
    pg.on('credentials_expired', () => this._handleCredsExpired(mid));
    ...
    const doPoll = async () => {
      ...
      try {
        ...
        if (/credentials/i.test(err.message)) {
          console.error(`Credentials expired for merchant ${mid}`);
        }
      } finally {
  ...
  getPaymentDetails(orderId) {
    ...
    return { ...payment, events, merchant: merchant ? { name: merchant.name, upiId: merchant.upi_id } : null };
  }
